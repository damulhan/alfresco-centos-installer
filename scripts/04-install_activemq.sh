...
ACTIVEMQ_HOME=/home/alfresco/activemq
...
fetch_latest_version() {
  ...
  curl -s https://dlcdn.apache.org/activemq/ | grep -oP '5+\.[0-9]+\.[0-9]+' | tail -1
}
...
echo "Creating alfresco user and group if they do not exist..."
if ! id -u $ACTIVEMQ_USER >/dev/null 2>&1; then
  sudo useradd -m $ACTIVEMQ_USER
fi
...
# Uncomment the next two lines if PostgreSQL is required
